...
/**
 * Repositorio para GastoObraProfesional
 */
@Repository
public interface GastoObraProfesionalRepository extends JpaRepository<GastoObraProfesional, Long> {
    ...
    List<GastoObraProfesional> findByProfesionalObraId(
    ...
    @Query("SELECT g FROM GastoObraProfesional g " +
           ...
           "AND g.empresaId = :empresaId " +
           "ORDER BY g.fechaHora DESC")
    List<GastoObraProfesional> findByDireccionObra(
    ...
    Optional<GastoObraProfesional> findByIdAndEmpresaId(
        @Param("id") Long id,
        @Param("empresaId") Long empresaId
    );
