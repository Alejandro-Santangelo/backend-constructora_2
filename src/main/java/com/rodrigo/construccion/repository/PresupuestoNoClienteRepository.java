...
public interface PresupuestoNoClienteRepository extends JpaRepository<PresupuestoNoCliente, Long>, JpaSpecificationExecutor<PresupuestoNoCliente> {
	 ...
	 */
	// @EntityGraph eliminado - las relaciones legacy 'profesionales' y 'materiales' no existen
	...
	// DEPRECADO: Este m\u00e9todo fue eliminado porque PresupuestoNoCliente ahora tiene relaci\u00f3n ManyToOne con Obra
	// En lugar de setear obraId, ahora se debe cargar la entidad Obra y setear con setObra(obra)
	/*
	@Modifying
	@Query("UPDATE PresupuestoNoCliente p SET p.obraId = :obraId, p.estado = :estado " +
	       "WHERE p.empresa.id = :empresaId AND " +
	       "LOWER(p.direccionObraCalle) = LOWER(:calle) AND " +
	       "p.direccionObraAltura = :altura")
	int updateObraIdAndEstadoByDireccion(
	...
	*/
	...
	List<PresupuestoNoCliente> findByEmpresaIdAndDireccionObraCalleIgnoreCaseAndDireccionObraAltura(
		Long empresaId,
		String calle,
		String altura
	);
